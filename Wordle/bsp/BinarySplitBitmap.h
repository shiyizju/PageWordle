...
public:
    BinarySplitBitmap (int iWidth, int iHeight, EnumDataFlag dataFlag = kDataFlagEmperty) : width(iWidth), height(iHeight) {
        assert(dataFlag != kDataFlagMixed);
        root = new BSPNode( { 0, 0, width, height } );
        root->dataFlag = dataFlag;
    }
    
    BinarySplitBitmap (int iWidth, int iHeight, const unsigned char* pixelData) : width(iWidth), height(iHeight) {
        root = new BSPNode( { 0, 0, width, height } );
    ...
    bool canAddBitmapAtEmpertyArea(MIRect rect, BinarySplitBitmap* bitmap) {
        return _canAddBitmapAtEmpertyArea(root, rect, bitmap, { 0, 0, rect.width, rect.height });
    }
    
    void addBitmapInRect(MIRect rect, BinarySplitBitmap* bitmap) {
...
private:
    ...
    bool _canAddBitmapAtEmpertyArea(BSPNode* node, MIRect rect, BinarySplitBitmap* bitmap, MIRect rectInBitmap);
    
    void _addBitmapInRect(BSPNode* node, MIRect rect, BinarySplitBitmap* bitmap, MIRect rectInBitmap);
